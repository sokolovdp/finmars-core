...
class CredentialsSerializer(ModelWithTimeStampSerializer):
    ...
    path_to_private_key = serializers.FileField(required=False, allow_null=True, allow_empty_file=False,
                                                write_only=True)
    ...
    def update(self, credentials, validated_data):
        ...
        if path_to_public_key:
            ...
            credentials.master_user.token, credentials.provider.user_code, path_to_public_key.name)
        ...
        if path_to_private_key:
            ...
            credentials.master_user.token, credentials.provider.user_code, path_to_private_key.name)
