...
def forwards_func(apps, schema_editor):
    ...
    complex_distinct_codes = ComplexTransaction.objects.values('code').annotate(min_id=Min('id'),
                                                                                num=Count("code")).filter(
        code__isnull=False, num__gt=1)

    print("going to delete %s complex transaction duplicates" % len(complex_distinct_codes))
    ...
    print("going to delete %s complex transaction duplicates DONE" % len(complex_distinct_codes))

    distinct_codes = Transaction.objects.values('transaction_code').annotate(min_id=Min('id'),
                                                                             num=Count("transaction_code")).filter(
        transaction_code__isnull=False, num__gt=1)
    ...
    for code_info in distinct_codes:
        ...
        # Get all objects with the same `code` except the first one (the one with the minimum id)
        duplicates = Transaction.objects.filter(transaction_code=transaction_code).exclude(id=min_id)
