...
class InviteToMasterUser(models.Model):
    ...
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        related_name="invites_to_master_user",
        verbose_name=gettext_lazy("user"),
        on_delete=models.CASCADE,
    )
    from_member = models.ForeignKey(
        Member,
        related_name="invites_to_users",
        verbose_name=gettext_lazy("from_member"),
        on_delete=models.CASCADE,
    )
    ...
    groups = models.ManyToManyField(
        "Group", blank=True, related_name="invites", verbose_name=gettext_lazy("groups")
    )

    status = models.IntegerField(default=0, choices=STATUS_CHOICES)

    class Meta:
        verbose_name = gettext_lazy("invite to master user")
        verbose_name_plural = gettext_lazy("invites to master user")
...
class Group(models.Model):
    ...
    permission_table_json_data = models.TextField(
        null=True, blank=True, verbose_name=gettext_lazy("json data"),
    )

    @property
    def permission_table(self):
        if not self.permission_table_json_data:
            return None
        try:
    ...
    def permission_table(self, val):
        if val:
            self.permission_table_json_data = json.dumps(
    ...
    def grant_all_permissions_to_model_objects(self, model, master_user, group):
        ...
        for item in model.objects.filter(master_user=master_user):
            perms = [{"group": group, "permission": p} for p in get_all_perms(item)]
            append_perms3(item, perms=perms)

    def grant_view_permissions_to_model_objects(self, model, master_user, group):
        from poms.obj_perms.utils import append_perms3, get_view_perms

        for item in model.objects.filter(master_user=master_user):
            perms = [{"group": group, "permission": p} for p in get_view_perms(item)]
            append_perms3(item, perms=perms)
    ...
    def grant_all_permissions_to_public_group(self, instance, master_user):
        ...
        self.grant_all_permissions_to_model_objects(
            Strategy1Group, master_user, instance
        )
        self.grant_all_permissions_to_model_objects(
            Strategy1Subgroup, master_user, instance
        )
        self.grant_all_permissions_to_model_objects(Strategy1, master_user, instance)

        self.grant_all_permissions_to_model_objects(
            Strategy2Group, master_user, instance
        )
        self.grant_all_permissions_to_model_objects(
            Strategy2Subgroup, master_user, instance
        )
        self.grant_all_permissions_to_model_objects(Strategy2, master_user, instance)

        self.grant_all_permissions_to_model_objects(
            Strategy3Group, master_user, instance
        )
        self.grant_all_permissions_to_model_objects(
            Strategy3Subgroup, master_user, instance
        )
        self.grant_all_permissions_to_model_objects(Strategy3, master_user, instance)

        self.grant_all_permissions_to_model_objects(
            GenericAttributeType, master_user, instance
        )

        self.grant_all_permissions_to_model_objects(
            InstrumentType, master_user, instance
        )
        self.grant_all_permissions_to_model_objects(Instrument, master_user, instance)

        self.grant_all_permissions_to_model_objects(
            TransactionTypeGroup, master_user, instance
        )
        self.grant_all_permissions_to_model_objects(
            TransactionType, master_user, instance
        )

        self.grant_all_permissions_to_model_objects(Portfolio, master_user, instance)

        self.grant_all_permissions_to_model_objects(
            CounterpartyGroup, master_user, instance
        )
        self.grant_all_permissions_to_model_objects(Counterparty, master_user, instance)

        self.grant_all_permissions_to_model_objects(
            ResponsibleGroup, master_user, instance
        )
        self.grant_all_permissions_to_model_objects(Responsible, master_user, instance)

        self.grant_all_permissions_to_model_objects(
            ComplexTransaction, master_user, instance
        )
