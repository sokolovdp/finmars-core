...
class AbstractProvider(object):
    ...
    def get_instrument_attribute_value(
        self, master_user, provider, attribute_type, value
    ):
        # if not value:
        #     return None
        if value is None:
            value = ""
        try:
            obj = InstrumentAttributeValueMapping.objects.select_related(
                "content_object", "classifier"
            ).get(
    ...
    def get_accrual_calculation_model(self, master_user, provider, value):
        ...
        if value is None:
            value = ""
        try:
            obj = AccrualCalculationModelMapping.objects.select_related(
                "content_object"
            ).get(master_user=master_user, provider=provider, value=value)
        ...
        try:
            ...
            for input in instrument_download_scheme.inputs.all():
                for key, value in values.items():
                    if input.name == key:
                        try:
                            values_converted[key] = formula.safe_eval(
                                input.name_expr, names=values
                            )
            ...
            try:
                ...
                errors["instrument_type_user_code"] = [
            ...
            default_instrument_object.update(
                {"user_code": user_code, "name": user_code}
            )
            ...
            try:
                instr = Instrument.objects.get(
                    master_user=master_user, user_code=user_code
                )
            except Instrument.DoesNotExist:
                serializer = InstrumentSerializer(
                    data=default_instrument_object, context=context
                )
            ...
            for attr in InstrumentDownloadScheme.BASIC_FIELDS:
                ...
                if attr in (
                    ...
                    if v:
                        # setattr(instr, attr, v)
                        self.set_instrument_attr(
                            instrument_download_scheme, instr, attr, v
                        )
                    ...
                    if self.is_empty_value(v):
                        ...
                        self.set_instrument_attr(
                            instrument_download_scheme, instr, instr_attr, v
                        )
            ...
            instr._accrual_calculation_schedules = (
                self.create_accrual_calculation_schedules(
                    instrument_download_scheme=instrument_download_scheme,
                    instrument=instr,
                    values=values,
                )
    ...
    def create_instrument_attributes(
        self, instrument_download_scheme, instrument, values, errors
    ):
        iattrs = []
        master_user = instrument_download_scheme.master_user
        provider = instrument_download_scheme.provider
        for attr in instrument_download_scheme.attributes.select_related(
            "attribute_type"
        ).all():
            if attr.attribute_type:
                tattr = attr.attribute_type

                iattr = GenericAttribute(
                    content_object=instrument, attribute_type=tattr
                )
                iattrs.append(iattr)
                ...
                if attr.value:
                    ...
                    attr_mapped_values = self.get_instrument_attribute_value(
                        master_user, provider, tattr, v
                    )
                        ...
                        if tattr.value_type == GenericAttributeType.STRING:
                            ...
                            if self.is_empty_value(v):
                                ...
                                try:
                                    ...
                                    errors[err_name] = [
                                        gettext_lazy("A valid number is required.")
                                    ]
                            ...
                            if self.is_empty_value(v):
                                ...
                                if isinstance(v, date):
                                    ...
                                    errors[err_name] = [
                                        gettext_lazy("A valid date is required.")
                                    ]
    ...
    def get_currency_history_values(self, price_scheme, values):
        ...
        return {
            "bid": self.price_adapt_value(
                value, price_scheme.currency_fxrate_multiplier
            ),
            "ask": self.price_adapt_value(
                value, price_scheme.currency_fxrate_multiplier
            ),
            "mid": self.price_adapt_value(
                value, price_scheme.currency_fxrate_multiplier
            ),
            "last": self.price_adapt_value(
                value, price_scheme.currency_fxrate_multiplier
            ),
        }
    ...
    def fail_pricing_policy(errors, pricing_policy, names):
        msg = gettext_lazy(
            'Invalid pricing policy expression in "%(pricing_policy)s".'
    ...
    def fail_manual_pricing_formula(errors, manual_pricing_formula, names):
        msg = gettext_lazy(
            'Invalid manual pricing formula expression in instrument "%(instrument)s".'
    ...
    if provider == ProviderClass.BLOOMBERG:
        if settings.BLOOMBERG_SANDBOX:
            ...
            try:
                ...
                try:
                    config = master_user.import_configs.get(
