...
class BloombergDataProvider(AbstractProvider):
    ...
    def soap_client(self):
        if not self._soap_client:
            ...
            # _l.debug('soap client: %s', self._soap_client)
    ...
    def get_instrument_send_request(self, instrument, fields):
        ...
        """
        _l.debug(
            '> get_instrument_send_request: instrument="%s", fields=%s',
            instrument,
            fields,
        )
        ...
        _l.debug(
            "request: instruments=%s, fields=%s, headers=%s",
            instruments,
            fields,
            headers,
        )
    ...
    def get_fields(self):
        ...
        response = self.soap_client.service.getFields(criteria={"mnemonic": "NAME"})
        _l.debug("response=%s", response)
        self._response_is_valid(response)
        return response
    ...
    def get_test_certificate_send_request(self):
        ...
        _l.info(
            "get_test_certificate_send_request.request: fields=%s, headers=%s",
            fields,
            headers,
        )
        response = self.soap_client.service.submitGetDataRequest(
        ...
        return {"is_authorized": is_authorized, "response_text": str(response)}
    ...
    def get_pricing_latest_send_request(self, instruments, fields):
        ...
        """
        _l.debug(
            "> get_pricing_latest_send_request: instrument=%s, fields=%s",
            instruments,
            fields,
        )
        ...
        response = self.soap_client.service.submitGetDataRequest(
            headers=headers, fields=fields_data, instruments=instruments_data
        )
        _l.debug("response=%s", response)
        self._response_is_valid(response)
    ...
    def get_pricing_latest_sync(self, instruments, fields):
        return self._invoke_sync(
            name="get_pricing_latest_sync",
            request_func=self.get_pricing_latest_send_request,
            request_kwargs={"instruments": instruments, "fields": fields},
    ...
    def get_pricing_history_send_request(self, instruments, fields, date_from, date_to):
        _l.debug(
            "> get_pricing_history_send_request: instrument=%s, fields=%s, date_from=%s, date_to=%s",
            instruments,
            fields,
            date_from,
            date_to,
        ...
        headers = {
            ...
            # "pricing": True,
            # "historical": True,
            "daterange": {"period": {"start": start, "end": end}}
        }
...
class FakeBloombergDataProvider(BloombergDataProvider):
    ...
    def get_pricing_latest_send_request(self, instruments, fields):
        _l.debug(
            "> get_pricing_latest_send_request: instruments=%s, fields=%s",
            instruments,
            fields,
        )
    ...
    def get_pricing_history_send_request(self, instruments, fields, date_from, date_to):
        _l.debug(
            "> get_pricing_history_send_request: instrument=%s, date_from=%s, date_to=%s",
            instruments,
            date_from,
            date_to,
        )
