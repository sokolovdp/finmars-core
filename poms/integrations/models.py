...
class Task(TimeStampedModel):
    ...
    provider = models.ForeignKey(ProviderClass, null=True, blank=True, db_index=True,
                                 verbose_name=gettext_lazy('provider'), on_delete=models.SET_NULL)
    action = models.CharField(max_length=20, db_index=True, verbose_name=gettext_lazy('action'))
    status = models.CharField(max_length=1, default=STATUS_PENDING, choices=STATUS_CHOICES,
                              verbose_name=gettext_lazy('status'))

    celery_tasks_id = models.CharField(max_length=255, blank=True, default='',
                                       verbose_name=gettext_lazy('celery tasks id'))
    ...
    request_id = models.CharField(max_length=50, null=True, db_index=True, verbose_name=gettext_lazy('request id'))
    response_id = models.CharField(max_length=50, null=True, db_index=True, verbose_name=gettext_lazy('response id'))
...
class PricingAutomatedSchedule(models.Model):
    ...
    is_enabled = models.BooleanField(default=True, verbose_name=gettext_lazy('is enabled'))
    ...
    balance_day = models.PositiveSmallIntegerField(default=0, verbose_name=gettext_lazy('balance day'))
    load_days = models.PositiveSmallIntegerField(default=1, verbose_name=gettext_lazy('load days'))
    fill_days = models.PositiveSmallIntegerField(default=0, verbose_name=gettext_lazy('tagfill dayss'))
    override_existed = models.BooleanField(default=True, verbose_name=gettext_lazy('override existed'))
    ...
    last_run_task = models.ForeignKey(Task, null=True, blank=True, on_delete=models.SET_NULL, editable=False,
                                      db_index=True, verbose_name=gettext_lazy('last run task'))

    class Meta:
        verbose_name = gettext_lazy('pricing automated schedule')
        verbose_name_plural = gettext_lazy('pricing automated schedules')
        index_together = (
    ...
    def __str__(self):
        return gettext_lazy('pricing automated schedule')
