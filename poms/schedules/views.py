...
from poms.schedules.serializers import PricingScheduleSerializer, RunScheduleSerializer, \
    TransactionFileDownloadScheduleSerializer

from poms.schedules.tasks import process_pricing_procedures_schedules
...
class TransactionFileDownloadScheduleFilterSet(FilterSet):
    user_code = CharFilter()
    name = CharFilter()
...
class TransactionFileDownloadScheduleViewSet(AbstractModelViewSet):
    queryset = TransactionFileDownloadSchedule.objects
    serializer_class = TransactionFileDownloadScheduleSerializer
    filter_class = TransactionFileDownloadScheduleFilterSet
    filter_backends = AbstractModelViewSet.filter_backends + [
        OwnerByMasterUserFilter,
    ]
    # permission_classes = AbstractModelViewSet.permission_classes + [
