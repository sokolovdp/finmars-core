...
def _get_strategy_system_attrs(strategy_number):
    ...
    return [
        ...
        {"key": "notes", "name": "Notes", "value_type": 10},
        {
            ...
            "name": "Group",
            "value_type": "field",
            "value_entity": f"strategy-{strategy_number}-subgroup",
            "value_content_type": f"strategies.strategy{strategy_number}subgroup",
            "code": "user_code",
        },
    ]
...
class Strategy1Subgroup(NamedModel, FakeDeletableModel, _SubgroupSystemAttrsMixin):
    master_user = models.ForeignKey(
        MasterUser,
        related_name="strategy1_subgroups",
        verbose_name=gettext_lazy("master user"),
        on_delete=models.CASCADE,
    )
...
class Strategy2Subgroup(NamedModel, FakeDeletableModel, _SubgroupSystemAttrsMixin):
    master_user = models.ForeignKey(
        MasterUser,
        related_name="strategy2_subgroups",
        verbose_name=gettext_lazy("master user"),
        on_delete=models.CASCADE,
    )
    group = models.ForeignKey(
...
class Strategy3Subgroup(NamedModel, FakeDeletableModel, _SubgroupSystemAttrsMixin):
    master_user = models.ForeignKey(
        MasterUser,
        related_name="strategy3_subgroups",
        verbose_name=gettext_lazy("master user"),
        on_delete=models.CASCADE,
    )
    group = models.ForeignKey(
