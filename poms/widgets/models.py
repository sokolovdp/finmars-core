...
class BalanceReportHistory(DataTimeStampedModel):
    ...
    accounts = models.ManyToManyField('accounts.Account', related_name='balance_report_histories', blank=True,
                                      verbose_name=gettext_lazy('accounts'))
    portfolios = models.ManyToManyField('portfolios.Portfolio', related_name='balance_report_histories', blank=True,
...
class BalanceReportHistoryItem(models.Model):
    ...
    balance_report_history = models.ForeignKey(BalanceReportHistory, related_name='items',
                                               verbose_name=gettext_lazy('balance report history'), on_delete=models.CASCADE)
