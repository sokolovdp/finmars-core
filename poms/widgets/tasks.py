...
def collect_balance_report_history(self, task_id):
    ...
    try:
        ...
        try:
            collect_asset_type_category(instance_serialized, balance_report_history, 'market_value')
        except Exception as e:
            _l.error("collect_balance_report_history. Could not collect asset type category")
        try:
            collect_currency_category(instance_serialized, balance_report_history, 'market_value')
        except Exception as e:
            _l.error("collect_balance_report_history. Could not collect currency category")
        try:
            collect_country_category(instance_serialized, balance_report_history, 'market_value')
        except Exception as e:
            _l.error("collect_balance_report_history. Could not collect country category")

        try:
            collect_region_category(instance_serialized, balance_report_history, 'market_value')
        except Exception as e:
            _l.error("collect_balance_report_history. Could not collect region category")

        try:
            collect_sector_category(instance_serialized, balance_report_history, 'market_value')
        except Exception as e:
            _l.error("collect_balance_report_history. Could not collect sector category")
