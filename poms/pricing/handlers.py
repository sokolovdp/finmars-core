...
from poms.pricing.models import (
    ...
    PricingProcedureBloombergCurrencyResult,
...
class PricingProcedureProcess:
    def __init__(
        ...
        _l.debug(
            f"PricingProcedureProcess. Master user: {master_user}. "
        ...
        _l.debug(f"price_date_from {self.procedure.price_date_from}")
        _l.debug("price_date_to %s" % self.procedure.price_date_to)

        self.parent_procedure = PricingParentProcedureInstance(
            procedure=procedure, master_user=master_user
        )
        self.parent_procedure.save()

        self.pricing_instrument_handler = PricingInstrumentHandler(
            procedure=procedure,
        ...
        self.pricing_currency_handler = PricingCurrencyHandler(
            procedure=procedure,
            parent_procedure=self.parent_procedure,
            master_user=master_user,
            member=self.member,
        ...
        _l.debug(
            "Procedure settings - Get Principal Prices: %s"
            % self.procedure.price_get_principal_prices
        )
        _l.debug(
            "Procedure settings - Get Accrued Prices: %s"
            % self.procedure.price_get_accrued_prices
        )
        _l.debug(
            "Procedure settings - Get FX Rates: %s" % self.procedure.price_get_fx_rates
        )
        _l.debug(
            "Procedure settings - Overwrite Principal Prices: %s"
            % self.procedure.price_overwrite_principal_prices
        )
        _l.debug(
            "Procedure settings - Overwrite Accrued Prices: %s"
            % self.procedure.price_overwrite_accrued_prices
        )
        _l.debug(
            "Procedure settings - Overwrite FX Rates: %s"
            % self.procedure.price_overwrite_fx_rates
        )
        _l.debug(
            "Procedure settings - Roll Days N Forward: %s"
            % self.procedure.price_fill_days
        )
    ...
    def get_base_transactions(self):
        ...
        _l.debug(
            "< get_base_transactions done in %s", (time.perf_counter() - processing_st)
        )
    ...
    def execute_procedure_date_expressions(self):
        if self.procedure.price_date_from_expr:
            try:
                self.procedure.price_date_from = formula.safe_eval(
                    self.procedure.price_date_from_expr, names={}
                )
        ...
        if self.procedure.price_date_to_expr:
            try:
                self.procedure.price_date_to = formula.safe_eval(
                    self.procedure.price_date_to_expr, names={}
                )
            except formula.InvalidExpression as e:
    ...
    def process(self):
        if (
            self.procedure.price_get_principal_prices
            or self.procedure.price_get_accrued_prices
        ):
...
class FillPricesBrokerBloombergProcess(object):
    def __init__(self, instance, master_user):
        ...
        self.procedure_instance = PricingProcedureInstance.objects.get(
            pk=self.instance["procedure"]
        )
        self.procedure = self.procedure_instance.procedure

        _l.debug(
            "Broker Bloomberg - Get Principal Prices: %s"
            % self.procedure.price_get_principal_prices
        )
        _l.debug(
            "Broker Bloomberg - Get Accrued Prices: %s"
            % self.procedure.price_get_accrued_prices
        )
        _l.debug(
            "Broker Bloomberg - Get FX Rates: %s" % self.procedure.price_get_fx_rates
        )
        _l.debug(
            "Broker Bloomberg - Overwrite Principal Prices: %s"
            % self.procedure.price_overwrite_principal_prices
        )
        _l.debug(
            "Broker Bloomberg - Overwrite Accrued Prices: %s"
            % self.procedure.price_overwrite_accrued_prices
        )
        _l.debug(
            "Broker Bloomberg - Overwrite FX Rates: %s"
            % self.procedure.price_overwrite_fx_rates
        )
        _l.debug(
            "Broker Bloomberg - Roll Days N Forward: %s"
            % self.procedure.price_fill_days
        )

    def process(self):
        _l.debug(
            "< fill prices: total items len %s" % len(self.instance["data"]["items"])
        )
        ...
        if self.instance["action"] == "bloomberg_get_currency_prices":
            for item in self.instance["data"]["items"]:
                ...
                _l.debug(
                    "< fill currency prices: records for %s len %s"
                    % (item["reference"], len(list(records)))
                )

                processing_st = time.perf_counter()

                _l.debug(
                    "< get records from db done: %s", (time.perf_counter() - records_st)
                )
                ...
                for record in records:
                    for field in item["fields"]:
                            ...
                            if str(record.date) == str(val_obj["date"]):
                                if record.fx_rate_parameters:
                                    if field["code"] in record.fx_rate_parameters:
                                        ...
                                        try:
                                            record.fx_rate_value_error_text = val_obj[
                                                "error_text"
                                            ]
                                        except Exception as e:
                                            _l.debug(
                                                "fx_rate_value_error_text e %s " % e
                                            )
                        ...
                        if not len(records):
                            _l.debug(
                                "Cant fill the value. Related records not found. Reference %s"
                                % item["reference"]
                            )
            ...
            for item in self.instance["data"]["items"]:
                ...
                _l.debug(
                    "< fill instrument prices: records for %s len %s"
                    % (item["reference"], len(list(records)))
                )
                            ...
                            if str(record.date) == str(val_obj["date"]):
                                if record.ask_parameters:
                                    if field["code"] in record.ask_parameters:
                                        ...
                                        try:
                                            record.ask_value_error_text = val_obj[
                                                "error_text"
                                            ]
                                        except Exception as e:
                                ...
                                if record.bid_parameters:
                                    if field["code"] in record.bid_parameters:
                                        ...
                                        try:
                                            record.bid_value_error_text = val_obj[
                                                "error_text"
                                            ]
                                        except Exception as e:
                                ...
                                if record.last_parameters:
                                    if field["code"] in record.last_parameters:
                                        ...
                                        try:
                                            record.last_value_error_text = val_obj[
                                                "error_text"
                                            ]
                                        except Exception as e:
                                ...
                                if record.accrual_parameters:
                                    if field["code"] in record.accrual_parameters:
                                        ...
                                        try:
                                            record.accrual_value_error_text = val_obj[
                                                "error_text"
                                            ]
                                        except Exception as e:
                                            _l.debug(
                                                "accrual_value_error_text e %s " % e
                                            )
    ...
    def create_price_history(self):
        ...
        for record in records:
            ...
            if (
                pricing_scheme_parameters.accrual_calculation_method == 2
            ...
            if (
                pricing_scheme_parameters.accrual_calculation_method == 3
            ):  # ACCRUAL_PER_FORMULA
                try:
            ...
            if can_write:
                ...
                if has_error:
                    ...
                    error.status = (
                        PriceHistoryError.STATUS_CREATED
                    )  # its journal, not error log
                    error.save()
            ...
            if self.instance["data"]["date_to"] == str(record.date):
                ...
                instrument_pp = record.instrument.pricing_policies.filter(
                    pricing_policy=record.pricing_policy
                )[0]
                ...
                roll_price_history_for_n_day_forward(
                    ...
                    price,
                    self.master_user,
                    self.procedure_instance,
                    instrument_pp,
                )

        PricingProcedureBloombergInstrumentResult.objects.filter(
            master_user=self.master_user,
            procedure=self.instance["procedure"],
            date__gte=self.instance["data"]["date_from"],
            date__lte=self.instance["data"]["date_to"],
        ...
        if self.procedure_instance.error_prices_count != 0:
            description = (
                "Price History Procedure finished with %s errors"
                % self.procedure_instance.error_prices_count
            )
            ...
            send_system_message(
                ...
                type="warning",
                title="Pricing Partial Error. Broker: Bloomberg. Procedure id %s"
                % self.procedure_instance.id,
        ...
        _l.debug(
            "bloomberg price self.procedure_instance.successful_prices_count %s"
            % self.procedure_instance.successful_prices_count
        )
        _l.debug(
            "bloomberg price self.procedure_instance.error_prices_count %s"
            % self.procedure_instance.error_prices_count
        )
    ...
    def create_currency_history(self):
        ...
        for record in records:
            ...
            safe_pp = {"id": record.pricing_policy.id}
            ...
            if can_write:
                if has_error or price.fx_rate == 0:
                    ...
                    error.status = (
                        CurrencyHistoryError.STATUS_CREATED
                    )  # its journal, not error log
                    error.save()
            ...
            if not can_write and exist:
                ...
                error.error_text = (
                    "Prices already exists. Fx rate: " + str(fx_rate) + "."
                )
            ...
            if parse_date_iso(self.instance["data"]["date_to"]) == record.date:
                ...
                successes, errors = roll_currency_history_for_n_day_forward(
                    record,
                    self.procedure,
                    price,
                    self.master_user,
        ...
        PricingProcedureBloombergCurrencyResult.objects.filter(
            master_user=self.master_user,
            procedure=self.instance["procedure"],
            date__gte=self.instance["data"]["date_from"],
            date__lte=self.instance["data"]["date_to"],
        ...
        if self.procedure_instance.error_prices_count != 0:
            description = (
                "Currency History Procedure finished with %s errors"
                % self.procedure_instance.error_prices_count
            )
...
class FillPricesBrokerBloombergForwardsProcess(object):
    def __init__(self, instance, master_user):
        ...
        _l.debug(
            "Broker Bloomberg Forwards - Get Principal Prices: %s"
            % self.procedure.price_get_principal_prices
        )
        _l.debug(
            "Broker Bloomberg Forwards - Get Accrued Prices: %s"
            % self.procedure.price_get_accrued_prices
        )
        _l.debug(
            "Broker Bloomberg Forwards - Get FX Rates: %s"
            % self.procedure.price_get_fx_rates
        )
        _l.debug(
            "Broker Bloomberg Forwards - Overwrite Principal Prices: %s"
            % self.procedure.price_overwrite_principal_prices
        )
        _l.debug(
            "Broker Bloomberg Forwards - Overwrite Accrued Prices: %s"
            % self.procedure.price_overwrite_accrued_prices
        )
        _l.debug(
            "Broker Bloomberg Forwards - Overwrite FX Rates: %s"
            % self.procedure.price_overwrite_fx_rates
        )
        _l.debug(
            "Broker Bloomberg Forwards - Roll Days N Forward: %s"
            % self.procedure.price_fill_days
        )
    ...
    def process(self):
        ...
        if self.instance["action"] == "bloomberg_forwards_get_instrument_prices":
            for item in self.instance["data"]["items"]:
                ...
                records = (
                    PricingProcedureBloombergForwardInstrumentResult.objects.filter(
                        master_user=self.master_user,
                        procedure=self.instance["procedure"],
                        reference=item["reference"],
                    )
                            ...
                            if str(record.date) == str(val_obj["date"]):
                                if record.price_code_parameters:
                                    if field["code"] in record.price_code_parameters:
                                        try:
                                            record.price_code_value = float(
                                                val_obj["value"]
                                            )
                                        except Exception as e:
                                            _l.debug("price_code_value %s " % e)
                                            _l.debug(
                                                "price_code_value original value %s "
                                                % val_obj["value"]
                                            )

                                        try:
                                            record.price_code_value_error_text = (
                                                val_obj["error_text"]
                                            )
                                        except Exception as e:
                                            _l.debug(
                                                "price_code_value_error_text e %s " % e
                                            )
    ...
    def create_price_history(self):
        ...
        _l.debug(
            "bloomberg forwards price procedure_instance %s" % self.procedure_instance
        )
        _l.debug(
            "bloomberg forwards price successful_prices_count %s"
        ...
        if self.procedure_instance.error_prices_count != 0:
            ...
            send_system_message(
                ...
                action_status="required",
                type="warning",
                title="Pricing Partial Error. Broker: Bloomberg Forwards. Procedure id %s"
                % self.procedure_instance.id,
                description=description,
            )
        ...
        _l.debug(
            "bloomberg forwards price self.procedure_instance.successful_prices_count %s"
            % self.procedure_instance.successful_prices_count
        )
        _l.debug(
            "bloomberg forwards price self.procedure_instance.error_prices_count %s"
            % self.procedure_instance.error_prices_count
        )
...
class FillPricesBrokerWtradeProcess(object):
    def __init__(self, instance, master_user):
        ...
        _l.debug(
            "Broker Wtrade - Roll Days N Forward: %s" % self.procedure.price_fill_days
        )
                        ...
                        for val_obj in field["values"]:
                            if str(record.date) == str(val_obj["date"]):
                                if field["code"] == "open":
                                    ...
                                    try:
                                        record.open_value_error_text = val_obj[
                                            "error_text"
                                        ]
                                    except Exception as e:
                                ...
                                if field["code"] == "close":
                                    ...
                                    try:
                                        record.close_value_error_text = val_obj[
                                            "error_text"
                                        ]
                                    except Exception as e:
                                ...
                                if field["code"] == "high":
                                    ...
                                    try:
                                        record.high_value_error_text = val_obj[
                                            "error_text"
                                        ]
                                    except Exception as e:
                                ...
                                if field["code"] == "low":
                                    ...
                                    try:
                                        record.low_value_error_text = val_obj[
                                            "error_text"
                                        ]
                                    except Exception as e:
                                ...
                                if field["code"] == "volume":
                                    ...
                                    try:
                                        record.volume_value_error_text = val_obj[
                                            "error_text"
                                        ]
                                    except Exception as e:
    ...
    def create_price_history(self):
        ...
        PricingProcedureWtradeInstrumentResult.objects.filter(
            master_user=self.master_user,
            procedure=self.instance["procedure"],
            date__gte=self.instance["data"]["date_from"],
            date__lte=self.instance["data"]["date_to"],
...
class FillPricesBrokerCbondsProcess(object):
    def __init__(self, instance, master_user):
        ...
        _l.debug(
            "Broker Cbonds - Roll Days N Forward: %s" % self.procedure.price_fill_days
        )
        ...
        if self.instance["action"] == "cbonds_get_instrument_prices":
            for item in self.instance["data"]["items"]:
                ...
                if not len(records):
                    ...
                    _l.debug(
                        "< processing item: %s", (time.perf_counter() - processing_st)
                    )
    ...
    def create_price_history(self):
        ...
        PricingProcedureCbondsInstrumentResult.objects.filter(
            master_user=self.master_user,
            procedure=self.instance["procedure"],
            date__gte=self.instance["data"]["date_from"],
            date__lte=self.instance["data"]["date_to"],
        ...
        if self.procedure_instance.error_prices_count != 0:
            ...
            send_system_message(
                ...
                action_status="required",
                type="warning",
                title="Pricing Partial Error. Broker: Finmars. Procedure id %s"
                % self.procedure_instance.id,
                description=description,
            )
...
class FillPricesBrokerFixerProcess(object):
    def __init__(self, instance, master_user):
        ...
        _l.debug(
            "Broker Fixer - Overwrite FX Rates: %s"
            % self.procedure.price_overwrite_fx_rates
        )
        _l.debug(
            "Broker Fixer - Roll Days N Forward: %s" % self.procedure.price_fill_days
        )
    ...
    def create_currency_history(self):
        ...
        PricingProcedureFixerCurrencyResult.objects.filter(
            master_user=self.master_user,
            procedure=self.instance["procedure"],
            date__gte=self.instance["data"]["date_from"],
            date__lte=self.instance["data"]["date_to"],
        ...
        _l.debug(
            "fixer self.procedure_instance.successful_prices_count %s"
            % self.procedure_instance.successful_prices_count
        )
        _l.debug(
            "fixer self.procedure_instance.error_prices_count %s"
            % self.procedure_instance.error_prices_count
        )
...
class FillPricesBrokerFxCbondsProcess(object):
    ...
    def create_currency_history(self):
        ...
        PricingProcedureCbondsCurrencyResult.objects.filter(
            master_user=self.master_user,
            procedure=self.instance["procedure"],
            date__gte=self.instance["data"]["date_from"],
            date__lte=self.instance["data"]["date_to"],
        ...
        _l.debug(
            "cbonds self.procedure_instance.successful_prices_count %s"
            % self.procedure_instance.successful_prices_count
        )
        _l.debug(
            "cbonds self.procedure_instance.error_prices_count %s"
            % self.procedure_instance.error_prices_count
        )
...
class FillPricesBrokerAlphavProcess(object):
    def __init__(self, instance, master_user):
        ...
        _l.debug(
            "Broker Alphav - Roll Days N Forward: %s" % self.procedure.price_fill_days
        )
    ...
    def create_price_history(self):
        ...
        PricingProcedureAlphavInstrumentResult.objects.filter(
            master_user=self.master_user,
            procedure=self.instance["procedure"],
            date__gte=self.instance["data"]["date_from"],
            date__lte=self.instance["data"]["date_to"],
