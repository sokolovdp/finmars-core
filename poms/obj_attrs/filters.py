...
class OwnerByAttributeTypeFilter(BaseFilterBackend):
    def filter_queryset(self, request, queryset, view):
        ...
        attribute_type_queryset = attribute_type_model.objects.filter(
...
class OrderingWithAttributesFilter(OrderingFilter):
    ...
    def get_valid_fields(self, queryset, view):
        valid_fields = super(OrderingWithAttributesFilter, self).get_valid_fields(
            queryset, view
        )
    ...
    def filter_queryset(self, request, queryset, view):
        ...
        if ordering:
            ...
            queryset = self.add_extra_fields(
                queryset, attr_model, self.get_attr_types(queryset, request), ordering
            )
    ...
    def get_attr_types(self, queryset, request):
        ...
        attr_type_qs = attr_type_model.objects.filter(
            master_user=request.user.master_user
        ).order_by("name", "pk")
