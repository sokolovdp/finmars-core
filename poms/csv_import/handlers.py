...
class SimpleImportProcess(object):
    ...
    def __init__(self, task_id, procedure_instance_id=None):
        ...
        if procedure_instance_id:
            ...
            _l.info(
                'SimpleImportProcess.Task %s. init procedure_instance %s' % (self.task, self.procedure_instance))
    ...
    def generate_file_report(self):
        ...
        _l.info('SimpleImportProcess.generate_file_report error_handler %s' % self.scheme.error_handler)
        _l.info(
            'SimpleImportProcess.generate_file_report missing_data_handler %s' % self.scheme.missing_data_handler)
        ...
        file_report.name = 'Simple Import %s (Task %s).csv' % (current_date_time, self.task.id)
    ...
    def generate_json_report(self):
        ...
        serializer = SimpleImportResultSerializer(instance=self.result, context=self.context)
        ...
        file_report.upload_file(file_name=file_name, text=json.dumps(result, indent=4, default=str),
                                master_user=self.master_user)
        file_report.master_user = self.master_user
        file_report.name = 'Simple Import %s (Task %s).json' % (current_date_time, self.task.id)
        file_report.file_name = file_name
        file_report.type = 'simple_import.import'
        file_report.notes = 'System File'
    ...
    def get_attribute_types(self):
        ...
        try:
            ...
            attribute_types = GenericAttributeType.objects.filter(master_user=self.master_user,
                                                                  content_type=self.scheme.content_type)
    ...
    def get_verbose_result(self):
        ...
        result = 'Processed %s rows and successfully imported %s items. Error rows %s' % (
    ...
    def fill_with_file_items(self):
        ...
        _l.info('SimpleImportProcess.Task %s. fill_with_raw_items INIT %s' % (self.task, self.process_type))
        ...
        try:
            ...
            _l.error('SimpleImportProcess.Task %s. fill_with_raw_items %s Exception %s' % (
                self.task, self.process_type, e))
            _l.error('SimpleImportProcess.Task %s. fill_with_raw_items %s Traceback %s' % (
            ...
            for scheme_input in self.scheme.csv_fields.all():
                ...
                try:
                    ...
                    if current_level == deep:
                        ...
                        _l.error('SimpleImportProcess.Task %s. recursive_preprocess init input %s Exception %s' % (
    ...
    def preprocess(self):
        ...
        _l.info(
            'SimpleImportProcess.Task %s. preprocess DONE items %s' % (self.task, len(self.preprocessed_items)))
                ...
                if entity_field.attribute_user_code:
                    ...
                    if entity_field.attribute_user_code == attribute_type.user_code:
                        ...
                        if attribute_type.value_type == GenericAttributeType.STRING:
                            attribute['value_string'] = item.inputs[entity_field.attribute_user_code]

                        if attribute_type.value_type == GenericAttributeType.NUMBER:
                            attribute['value_float'] = item.inputs[entity_field.attribute_user_code]
                        ...
                        if attribute_type.value_type == GenericAttributeType.DATE:
                            attribute['value_date'] = item.inputs[entity_field.attribute_user_code]
    ...
    def process_items(self):
        ...
        for item in self.items:
            ...
            try:
                ...
                _l.info('SimpleImportProcess.Task %s. ========= process row %s/%s ========' % (
                ...
                _l.error('SimpleImportProcess.Task %s.  ========= process row %s ======== Exception %s' % (
                    self.task, str(item.row_number), e))
                _l.error('SimpleImportProcess.Task %s.  ========= process row %s ======== Traceback %s' % (
    ...
    def process(self):
        ...
        try:
            ...
            _l.error('SimpleImportProcess.Task %s. process Exception %s' % (self.task, e))
            _l.error('SimpleImportProcess.Task %s. process Traceback %s' % (self.task, traceback.format_exc()))
            ...
            system_message_description = 'New items created (Import scheme - ' + str(
