...
class Tag(NamedModel):
    master_user = models.ForeignKey('users.MasterUser', related_name='tags', verbose_name=gettext_lazy('master user'), on_delete=models.CASCADE)
    content_types = models.ManyToManyField(ContentType, related_name='tags', blank=True,
                                           verbose_name=gettext_lazy('content types'))

    # account_types = models.ManyToManyField('accounts.AccountType', related_name='tags', blank=True,
    #                                        verbose_name=gettext_lazy('account types'))
    ...
    #                                              verbose_name=gettext_lazy('counterparty groups'))
    ...
    #                                             verbose_name=gettext_lazy('responsible groups'))
    ...
    #                                                  blank=True, verbose_name=gettext_lazy('transaction type groups'))
...
#                                        verbose_name=gettext_lazy('content object'))
#
#     class Meta(AbstractUserObjectPermission.Meta):
#         verbose_name = gettext_lazy('tags - user permission')
#         verbose_name_plural = gettext_lazy('tags - user permissions')
#
#
...
#         verbose_name = gettext_lazy('tags - group permission')
#         verbose_name_plural = gettext_lazy('tags - group permissions')


class TagLink(models.Model):
    tag = models.ForeignKey(Tag, related_name='links', verbose_name=gettext_lazy('tag'), on_delete=models.CASCADE)

    content_type = models.ForeignKey(ContentType, null=True, blank=True, verbose_name=gettext_lazy('content type'), on_delete=models.CASCADE)
    object_id = models.BigIntegerField(verbose_name=gettext_lazy('object id'))
    content_object = GenericForeignKey('content_type', 'object_id')

    class Meta:
        verbose_name = gettext_lazy('tag link')
        verbose_name_plural = gettext_lazy('tag links')
